    ...
    try {
        ...
        try {
          ...
          `,
            ...
            await Logger.error("createTicket", "Failed to send email notification", { message: emailError.message });
        }
...
exports.updateTicket = async (req, res, next) => {
    try {
        ...
        await Logger.info({
            ...
            status: "Success",
            details: `Ticket updated successfully (ID: ${ticketId})`,
        });
...
exports.updateTicketStatus = async (req, res) => {
    try {
        ...
        if (req.user.role !== "admin") { // system admin bypass
            if (!ticketTenantId || !userTenantId) {
                return res.status(400).json({ message: "Tenant info missing" });
            }
            if (ticketTenantId !== userTenantId) {
        ...
        // ✅ Success log (only when 200/201 response)
        await Logger.info({
            user: req.user._id,
            action: "Update Ticket Status",
            status: "Success",
            details: `Ticket status updated successfully (ID: ${ticket._id}, Status: ${ticket.status})`,
        });
